...
def __draw_lines(d, cds, x_transform, y_transform, x="x", y="y", color="c",
                      conv_coords_x=__conv_coords, conv_coords_y=__conv_coords):
    for xs, ys, c, in zip(cds[x], cds[y], cds[color]):
        xs2, ysy = [[conv_coords(p, is_bottom, *transform) for p in ps] for ps, is_bottom, transform, conv_coords in [
...
def __draw_heatmap(session, d, sizes, print_callback=lambda s: None):
    if sizes["show_heat"]:
        ...
        d.append(drawSvg.Rectangle(offset, offset, sizes["heatmap"], sizes["heatmap"], 
                    fill=session.get_background_color(print_callback)))

        x_transform, y_transform = __get_transform(session, sizes["heatmap"], offset, sizes["heatmap"], offset)

        __draw_rectangles(d, session.get_heatmap(print_callback), x_transform, y_transform, 
                        left="screen_left", right="screen_right",
                        bottom="screen_bottom", top="screen_top", color="color")
...
def __draw_annotation(session, d, sizes, print_callback=lambda s: None):
    if sizes["show_anno"]:
        ...
        d.append(drawSvg.Rectangle(offset_x, offset, sizes["heatmap"], 
                                    sizes["annotation"], fill="#ffffff"))
        d.append(drawSvg.Rectangle(offset, offset_x, sizes["annotation"], 
                                    sizes["heatmap"], fill="#ffffff"))

        x_transform, y_transform = __get_transform(session, sizes["heatmap"], offset_x, sizes["heatmap"], offset_x)
        active_anno_x = [sizes["annotation"], offset, session.get_displayed_annos(True, print_callback)]
        active_anno_y = [sizes["annotation"], offset, session.get_displayed_annos(False, print_callback)]
        
        __draw_rectangles(d, session.get_annotation(True, print_callback), x_transform, active_anno_x, 
                        left="screen_start", right="screen_end",
                        bottom="anno_name", top="anno_name", color="color",
                        conv_coords_y=__cat_coords)
        __draw_rectangles(d, session.get_annotation(False, print_callback), active_anno_y, y_transform, 
                        bottom="screen_start", top="screen_end",
                        left="anno_name", right="anno_name", color="color",
...
def __draw_secondary(session, d, sizes, print_callback=lambda s: None):
    if sizes["show_secondary"]:
        ...
        d.append(drawSvg.Rectangle(offset_x, offset, sizes["heatmap"], 
                                    sizes["secondary"], fill="#ffffff"))
        d.append(drawSvg.Rectangle(offset, offset_x, sizes["secondary"], 
                                    sizes["heatmap"], fill="#ffffff"))
        ...
        __draw_lines(d, session.get_tracks(True, print_callback), x_transform, active_anno_x, 
                        x="screen_pos", y="values", color="colors")
        __draw_lines(d, session.get_tracks(False, print_callback), active_anno_y, y_transform, 
                        y="screen_pos", x="values", color="colors")
...
def __get_sizes(session):
    return {
        ...
        "show_anno": session.get_value(["settings", "interface", "show_hide", "annotation"]),
        "annotation": session.get_value(["settings", "interface", "anno_size", "val"]),
        "show_secondary": session.get_value(["settings", "interface", "show_hide", "raw"]),
