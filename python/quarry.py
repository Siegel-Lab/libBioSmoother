...
class Quarry(PartialQuarry):
    def normalizeBinominalTestTrampoline(self, bin_values, num_interactions_total, num_bins, p_accept):
        ps = [binom_test(x, num_interactions_total, 1/num_bins, alternative='greater') for x in bin_values]
    ...
    def colorPalette(self, palette_name, color_low, color_high);
        if palette_name == "Viridis256":
            ...
            return [self.__combine_hex_values({color_low[1:]: 1-x/255, color_high[1:]: x/255}) for x in range(256)]
        else:
